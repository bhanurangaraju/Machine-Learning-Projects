function [Accuracy,TotalBatchTime]=SVM()
disp('SVM');
BatchTime=tic;                                  % Start Time
load heart.txt                                  % load the dataset
data = heart;
data(find(data(:,end)==1),end)=-1;
data(find(data(:,end)==2),end)=1;
...
function [ output ] = kernel(wholeInputs, input)
